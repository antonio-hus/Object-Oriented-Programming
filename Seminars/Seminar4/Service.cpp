...
void Service::addSongToRepository(const std::string& artist, const std::string& title, double minutes, double seconds, const std::string& source)
{
    ...
    this->undoStack.push_back(std::make_unique<ActionAdd>(s, repo));
}
...
void Service::removeSongFromRepository(const std::string & artist, const std::string & title)
{
    ...
    this->undoStack.push_back(std::make_unique<ActionRemove>(s, repo));
}
...
void Service::undo() {
    auto_ptr a = this->undoStack.pop_back();
    a->executeUndo();
    this->redoStack.push_back(a);
}

void Service::redo() {
    unique_ptr<Action> a = this->undoStack.pop_back();
    a->executeRedo();
}
